...
type PickCustom<T, K extends keyof T> = {
  // keyof T 를 풀어보면 K extends 'title' | 'tags' | 'content' | 'thumbnailURL'
...
// Pick<Post, Exclude<'title' | 'content' | 'tags' | 'thumbnailURL', 'title'>> // Exclude -> 특정 타입 제거하는 타입
...
- 객체 타입을 만들어주는 유틸리티 타입으로 겍채의 프로퍼티 키를 value 는 키들의 값
- 레코드 타입을 이용하면 동일한 패턴을 갖는 객체 타입을 쉽게 정의 가능
